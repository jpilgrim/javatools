 ...
 * See {@link If}.
 */
public class Fi extends PrimitiveCommand {

	@Override
	public Iterator<Token> expand(ProcessorState state, ITokenIterator tokens) {
		Optional<ConditionResult> optCr = state.popConditionResult();
		if (! optCr.isPresent()) {
			throw new ExpansionError(tokens.getLocation(), "Found \\fi without preceeding \\if");
		}
